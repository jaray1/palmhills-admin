...
Class Action {
	...
	function login(){
			...
			$qry = $this->db->query("SELECT * FROM users where username = '$username' and password = '$encryptedPassword' ");
			// echo md5($password);
			// echo $qry->num_rows;
			if($qry->num_rows > 0){
	...
	function save_user(){
		...
		// 	$establishment_id = 0;
		// } else {
		// 	$establishment_id = 1; // added else equal to 1 to fix undefined variable (14/02/2022)
		// }
		...
		echo $data;
		if(empty($id)){
			$save = $this->db->query("INSERT INTO users set ".$data); // repeating action thrice
		}else{
